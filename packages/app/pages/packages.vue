...
import type { PackageInfo, SessionContext } from '@rolldown-analyzer/core/types/data';
...
const { packages: packagesData } = useData();
...
const normalizedPackages = computed(() => {
  ...
  const sortedPackages = packagesSizeSortType
    ? data.sort((a, b) =>
        packagesSizeSortType === 'asc'
          ? a.transformedCodeSize - b.transformedCodeSize
          : b.transformedCodeSize - a.transformedCodeSize,
      )
    : data;
  ...
  return sortedPackages.filter(
    (item) =>
      ...
      searchValue.value.selected.some((rule) => rule.match(item.type!)),
  );
});
...
const { tree, chartOptions, graph, nodeHover, nodeSelected, selectedNode, selectNode, buildGraph } =
  useChartGraph<PackageInfo, PackageChartInfo, PackageChartNode>({
    ...
    graphOptions: {
      onHover(node) {
        if (node && !route.query.package) nodeHover.value = node;
        if (node === null) nodeHover.value = undefined;
      },
      onClick(node) {
        if (node.meta?.type === 'package') {
          router.replace({
            query: { ...route.query, package: `${node.meta?.name}@${node.meta?.version}` },
          });
        }
      },
      onLeave() {
...
watch(
  () => settings.value.packageViewType,
  () => {
    buildGraph();
  },
);
  ...
  <div relative max-h-screen of-hidden>
    ...
    <div of-auto h-screen flex="~ col gap-2" pt44 px4 pb4>
      ...
      <template v-else-if="settings.packageViewType === 'treemap'">
        ...
        <span v-else w-full h-48 flex="~ items-center justify-center" op50 italic> No Data </span>
      </template>
    ...
    <DisplayGraphHoverView :hover-x="mouse.x" :hover-y="mouse.y">
      <div v-if="nodeHover?.meta" bg-glass border="~ base rounded" p2 text-sm flex="~ col gap-2">
