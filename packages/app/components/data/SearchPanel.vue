...
function isRuleSelected(rule: FilterMatchRule) {
  ...
  if (!modelValue?.selected) return true;
...
function reverseSelect() {
  if (model?.value?.selected?.length === props.rules.length) {
    ...
    model.value.selected = props.rules
      .map((r) => r.name)
      .filter((r) => !model.value.selected?.includes(r));
  }
}
  ...
  <div flex="col gap-2" max-w-90vw min-w-30vw border="~ base rounded-xl" bg-glass>
    <slot name="search">
      <div v-if="modelValue.search !== false" class="flex items-center">
        <input v-model="model.search" p2 px4 w-full style="outline: none" placeholder="Search" />
    ...
    <div v-if="rules.length" :class="selectedContainerClass" flex="~ gap-2 wrap" p2 border="t base">
      ...
      >
        <input type="checkbox" mr1 :checked="isRuleSelected(rule)" @change="toggleRule(rule)" />
