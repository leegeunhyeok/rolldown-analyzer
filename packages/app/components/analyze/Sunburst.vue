...
const props = defineProps<{
  ...
  selected?: AnalyzeChartNode | undefined;
}>();

const emit = defineEmits<{
  (e: 'select', node: AnalyzeChartNode | null): void;
}>();
      ...
      <div v-if="selected" grid="~ cols-[250px_1fr] gap-1">
        <template v-for="child of selected.children" :key="child.id">
          ...
          >
            ...
            <span v-else>{{ child.text || child.id }}</span>
          </button>
          ...
          >
            ...
            <div
              ...
              v-tooltip="`${child.children.length} children`"
              :title="`${child.children.length} children`"
              text-xs
              op-fade
            >
