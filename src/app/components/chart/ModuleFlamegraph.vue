...
const tree = computed(() => {
  const resolveIds = props.info.resolve_ids.map((id, idx) =>
    n({
      id: `resolveId-${idx}`,
      text: id.plugin_name,
      size: id.duration,
  ...
  const loads = props.info.loads.map((load, idx) =>
    n({
      id: `load-${idx}`,
  ...
  const transforms = props.info.transforms.map((transform, idx) =>
    n({
      id: `transform-${idx}`,
