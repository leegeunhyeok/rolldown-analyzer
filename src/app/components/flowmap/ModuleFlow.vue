...
<template>
  <Splitpanes
    v-if="selected"
    class="!h-auto w-full module-flow-splitter right-4 bottom-4 fixed z-10 pointer-events-none"
  >
    ...
    <Pane size="55" min-size="10" max-size="90" class="!h-auto !of-visible">
      ...
      <div
        v-on-click-outside="[
          handleClose,
          { ignore: ['.splitpanes__splitter', '.flowmap-node-inline'] },
        ]"
        w-full
        h="[calc(100vh-(var(--spacing)*58))]"
        pointer-events-auto
        sticky
        top-4
      >
        <div
          ...
          cursor-col-resize
        />
        <ModuleFlowDetails :selected="selected" :session="session" @close="handleClose" />
      </div>
    </Pane>
  </Splitpanes>
  <div p4>
