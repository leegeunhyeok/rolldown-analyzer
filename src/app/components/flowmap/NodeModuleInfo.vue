...
function enter() {
  if (isFlowmapSwapping.value) emit('select', props.item);
}
...
const importterModule = computed(() => {
  if (props.item.type !== 'resolve') return undefined;
  ...
  >
    ...
    <template #inline-after>
      ...
      <template v-if="item.type === 'transform'">
        <div v-if="item.content_from === item.content_to" text-xs op25>no changes</div>
        <div v-else>
    ...
    <template v-if="item.type === 'resolve' && item.resolved_id && item.importer" #after>
      <div
        p3
        ml4
        border="l"
      ...
      >
        ...
        <DisplayModuleId :id="item.module_request" :session="session" />
      </div>
    </template>
  </FlowmapNode>
</template>
