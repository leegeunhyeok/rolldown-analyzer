...
import DisplayIconButton from '../../../_devtools-ui/components/DisplayIconButton.vue';
...
const codeDisplay = computed(() => {
  ...
  if (!('type' in props.selected)) return null;
  ...
  >
    <template v-if="selected?.type === 'chunk'">
      <div p4 h-full of-auto style="overscroll-behavior: contain">
        <DataChunkDetails :chunk="selected" :session="session" />
      </div>
    </template>
    <template v-else-if="selected?.type === 'asset'">
      <div p4 h-full of-auto style="overscroll-behavior: contain">
        <DataAssetDetails :asset="selected" :session="session" :lazy="true" />
      </div>
    </template>
    ...
    <span v-else op50 italic ma> No data </span>
  </div>
</template>
