...
const { state: showTypeState, next: _toggleShowType } = useCycleList<
  ClientSettings['pluginDetailsShowType']
>(['changed', 'unchanged', 'all'], {
  initialValue: settings.value.pluginDetailsShowType,
});

const showTypeText = computed(() => {
  if (showTypeState.value === 'all') return 'Show Changed';
  if (showTypeState.value === 'changed') return 'Show Unchanged';
  return 'Show All';
});
const showUnchangedInfo = computed(() => {
  if (!['load', 'transform'].includes(settings.value.pluginDetailSelectedHook)) return false;

  if (
    !props.buildMetrics.calls
      ?.filter((i) => i.type === settings.value.pluginDetailSelectedHook)
      .filter((i) => i.unchanged).length
  )
    return false;
  return true;
});
const unchangedInfo = computed(() => {
  const unchanged = props.buildMetrics.calls
    ?.filter((i) => i.type === settings.value.pluginDetailSelectedHook)
  ...
  <div p2 h-full w-full>
    <div flex="~" border="~ base" rounded-2 h-full relative of-hidden>
      <div v-if="expanded" of-hidden border="r base">
        <FlowmapPluginFlowTimeline :session="session" :build-metrics="buildMetrics">
          <template #header>
        ...
        >
          <div flex="~ items-center" h-full>
            ...
            <DataSearchPanel
              ...
              h-full
              border-none
              selected-container-class="px0! py1 border-none bg-none flex-nowrap! h-full"
              :rules="tableFieldFilterRules"
              class="[&_[icon-catppuccin]]:(filter-none!)"
            />
          </div>
          <div
            v-if="showUnchangedInfo"
            flex="~ items-center justify-center gap1"
            h-full
            text-xs
            py1
          >
            <p class="op50" flex="~ items-center gap-1">
              <DisplayNumberBadge :number="unchangedInfo.count" /> module unchanged, but cost
              <DisplayDuration :duration="unchangedInfo.duration" />
            </p>
            <button
              rounded-md
              px2
