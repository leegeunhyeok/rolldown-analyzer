...
const transforms = computed(
  (): (RolldownModuleNoChanges | RolldownModuleNoChangesHide | RolldownModuleTransformInfo)[] => {
    ...
    if (flowShowAllTransforms.value && !unchanged?.length) return info?.value?.transforms ?? [];
...
const loads = computed(() => {
  ...
  if (flowShowAllLoads.value && !unchanged?.length) return info?.value?.loads ?? [];
...
<template>
  <div select-none h-full of-auto ws-nowrap w-100vh of-visible>
    ...
    <div v-if="info.importers?.length" text-sm>
      ...
      <div pl-10 border="r" h-4 w-1px z-flowmap-line class="border-flow-line border-dashed" />
    </div>

    <!-- Main module node -->
    <div flex="~">
      <FlowmapNode :lines="{ bottom: true }" :active="selected != null">
        <template #content>
          <div p2>
            <DisplayModuleId :id="info.id" :session="session" />
          </div>
        </template>
      </FlowmapNode>
    ...
    >
      <template #node>
        <div i-ph-upload-simple-duotone />
        Load
