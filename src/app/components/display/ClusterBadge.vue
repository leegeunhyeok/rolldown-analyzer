...
import { getHashColorFromString } from '../../../_devtools-ui/utils/color';
...
<template>
  <Tooltip>
    <div flex="~ gap-1 items-center" text-sm pl1 pr2 rounded border-l-3 border :style>
      <div v-if="parsed.namespace" text-xs op-fade>{{ parsed.namespace }}:</div>
    ...
    <template #popper>
      <div>
        <div v-if="parsed.namespace === 'catalog'">
          Introduced by packages marked as
          <code :style="{ color }">{{ parsed.value }}</code> catalog in pnpm-workspace.yaml
        </div>
        <div v-else>
