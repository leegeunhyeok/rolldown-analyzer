...
const nodesHeight = computed(
  () =>
    SPACING.height * normalizedMaxLinks.value +
    SPACING.padding * (normalizedMaxLinks.value + 1) +
    SPACING.border * 2,
);
...
const dotNodeMargin = computed(
  () =>
    `${nodesHeight.value / 2 - SPACING.dot / 2}px ${SPACING.dotOffset}px 0  ${props.importers?.length ? SPACING.dotOffset : 0}px`,
);
const linkStartX = computed(() =>
  props.importers?.length ? SPACING.width + SPACING.marginX : SPACING.marginX,
);
const dotStartX = computed(() =>
  props.importers?.length ? linkStartX.value + SPACING.dotOffset : linkStartX.value,
);
const dotStartY = computed(
  () =>
    (SPACING.height * normalizedMaxLinks.value + (normalizedMaxLinks.value + 1) * SPACING.padding) /
    2,
);
...
function calculateLinkX(type: LinkPoint) {
  switch (type) {
    ...
    case 'import-end':
      return props.importers?.length
        ? linkStartX.value + SPACING.dotOffset * 2 + SPACING.dot
        : linkStartX.value + SPACING.dotOffset + SPACING.dot;
  }
}
...
function calculateLinkY(type: LinkPoint, i?: number) {
  switch (type) {
    case 'importer-start':
      return (
        (SPACING.height + SPACING.padding) * i! +
        (SPACING.height / 2 + SPACING.padding) +
        importersVerticalOffset.value
      );
    case 'import-end':
      return (
        (SPACING.height + SPACING.padding) * i! +
        (SPACING.height / 2 + SPACING.padding) +
        importsVerticalOffset.value
      );
    case 'importer-end':
    case 'import-start':
...
onMounted(() => {
  watch(() => [props.importers, props.imports], generateLinks, { immediate: true });
});
</script>

<template>
  <div v-if="importers?.length || imports?.length" ref="container" w-full relative select-none>
    <!-- nodes -->
    <div flex px2>
      <!-- importers -->
      <div
