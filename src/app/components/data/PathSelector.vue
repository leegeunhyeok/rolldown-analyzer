...
defineSlots<{
  list: (props: { select: (module: T) => void; data: T[] }) => void;
  item: (props: { id: string }) => void;
}>();
...
const startSelector: GraphPathSelector<T> = useGraphPathSelector<T>({
  ...
  getModules: () => {
    if (!startSelector.state.value.search) {
      ...
      return (
        startSelector.fuse
          .value!.value?.search(startSelector.state.value.search)
          .map((r) => r.item) ?? []
      );
    }
  },
...
function getAllImports(moduleId: string, visited = new Set<string>()): T[] {
  ...
  if (!module?.imports?.length) return [];
...
const endSelector = useGraphPathSelector<T>({
  searchKeys: props.searchKeys,
  getModules: () => {
    return startSelector.state.value.selected
      ? getAllImports(startSelector.state.value.selected)
      : [];
  },
});
...
const filteredEndModules = computed(() => {
  if (!endSelector.state.value.search) {
    ...
    return (
      endSelector.fuse.value!.value?.search(endSelector.state.value.search).map((r) => r.item) ?? []
    );
  }
});
      ...
      >
        ...
        <template #empty>
          <div flex="~ items-center justify-center" w-full h-20>
            <span italic op50>
              {{
                startSelector.state.value.selected
                  ? 'No modules'
                  : 'Select a start module to get end modules'
              }}
            </span>
          </div>
        </template>
