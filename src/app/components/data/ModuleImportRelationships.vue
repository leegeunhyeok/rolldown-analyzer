...
const normalizedMaxLinks = computed(() => {
  return Math.min(
    Math.max(props.module.importers?.length || 0, props.module.imports?.length || 0),
    MAX_LINKS,
  );
});
...
const dotNodeMargin = computed(
  () =>
    `${nodesHeight.value / 2 - SPACING.dot / 2}px ${SPACING.dotOffset}px 0  ${importers.value?.length ? SPACING.dotOffset : 0}px`,
);
const linkStartX = computed(() =>
  importers.value?.length ? SPACING.width + SPACING.marginX : SPACING.marginX,
);
const dotStartX = computed(() =>
  importers.value?.length ? linkStartX.value + SPACING.dotOffset : linkStartX.value,
);
...
onMounted(() => {
  watch(() => [props.module], generateLinks, { immediate: true });
});
</script>
