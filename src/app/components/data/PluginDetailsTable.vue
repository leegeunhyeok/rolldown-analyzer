...
import DataVirtualList from '../../../_devtools-ui/components/DataVirtualList.vue';
import DisplayBadge from '../../../_devtools-ui/components/DisplayBadge.vue';
...
const filterModuleTypes = ref<string[]>(
  settings.value.pluginDetailsModuleTypes ?? searchFilterTypes.value.map((i) => i.name),
);
...
const filtered = computed(() => {
  ...
  return sorted
    .filter((i) => {
      if (!i.module) return false;
      const matched = getFileTypeFromModuleId(i.module);
      return filterModuleTypes.value.includes(matched.name);
    })
    .filter(
      settings.value.pluginDetailSelectedHook
        ? (i) => i.type === settings.value.pluginDetailSelectedHook
        : Boolean,
    )
    .filter((i) => {
      if (
        settings.value.pluginDetailsShowType === 'all' ||
        !['load', 'transform'].includes(settings.value.pluginDetailSelectedHook)
      )
        return true;
...
<template>
  <div role="table" min-w-max>
    <div role="row" class="sticky top-0 z10 border-b border-base" flex="~ row">
      <div
        v-if="selectedFields.includes('hookName')"
        role="columnheader"
        bg-base
        flex-none
        w32
      ...
      <div
        v-if="selectedFields.includes('module')"
        role="columnheader"
        bg-base
        flex-1
        min-w100
          ...
          <VMenu>
            ...
            >
              <i
                ...
                :class="
                  filterModuleTypes.length !== searchFilterTypes.length
                    ? 'text-primary op100'
                    : 'op50'
                "
              />
      ...
      <div
        v-if="selectedFields.includes('duration')"
        role="columnheader"
        rounded-tr-2
        bg-base
        flex-none
      ...
      >
        <button
          flex="~ row gap1 items-center justify-center"
          w-full
          @click="toggleDurationSortType"
        >
          Duration
          <span
            w-6
          ...
          >
            <i
              text-xs
              :class="[
                durationSortType !== 'asc' ? 'i-ph-arrow-down-duotone' : 'i-ph-arrow-up-duotone',
                durationSortType ? 'op100 text-primary' : 'op50',
              ]"
            />
          </span>
        </button>
      ...
      <div
        v-if="selectedFields.includes('startTime')"
        role="columnheader"
        rounded-tr-2
        bg-base
        flex-none
      ...
      <div
        v-if="selectedFields.includes('endTime')"
        role="columnheader"
        rounded-tr-2
        bg-base
        flex-none
    ...
    <div v-else>
      <div p4>
        <div w-full h-48 flex="~ items-center justify-center" op50 italic>
          <p v-if="!selectedFields.length">No columns selected</p>
          <p v-else>No data</p>
        </div>
      </div>
    </div>
