...
// In standalone mode, use chunks from props or from useData() instead of RPC
const { chunks: dataChunks } = useData();
const normalizedChunks = computed(() => props.chunks || dataChunks.value);
...
const importers = computed((): RolldownChunkImport[] => {
  return (
    normalizedChunks.value
      ?.filter((c) => c.imports.some((i) => i.chunk_id === props.chunk.chunk_id))
      .map((chunk) => {
...
<template>
  <div flex="~ col gap-3">
    ...
    <template v-if="showDetails">
      ...
      <template v-if="normalizedChunks">
