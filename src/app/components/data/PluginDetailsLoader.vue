...
import DisplayCloseButton from '../../../_devtools-ui/components/DisplayCloseButton.vue';
import DisplayDuration from '../../../_devtools-ui/components/DisplayDuration.vue';
import DisplayNumberBadge from '../../../_devtools-ui/components/DisplayNumberBadge.vue';
import { formatDuration } from '../../../_devtools-ui/utils/format';
...
const { getPluginDetails } = useData();
const state = computed(() => getPluginDetails(+(route.query.plugin as string)));

const processedModules = computed(() => {
  const seen = new Set();
  return (
    state.value?.calls?.filter((call) => {
...
const hookLoadDuration = computed(() =>
  state.value?.loadMetrics.reduce((arc, item) => arc + item.duration, 0),
);

const hookTransformDuration = computed(() =>
  state.value?.transformMetrics.reduce((arc, item) => arc + item.duration, 0),
);

const hookResolveIdDuration = computed(() =>
  state.value?.resolveIdMetrics.reduce((arc, item) => arc + item.duration, 0),
);

const totalDuration = computed(() =>
  state.value?.calls?.reduce((arc, item) => arc + item.duration, 0),
);

const parsedPaths = computed(() =>
  props.session.modulesList.map((mod) => {
...
const moduleTypes = computed(() =>
  ModuleTypeRules.filter((rule) => parsedPaths.value.some((mod) => rule.match.test(mod.mod.id))),
);
...
const _tree = computed(() => {
  ...
  function createNode(node: TreeNodeInput<PluginChartInfo | undefined>, parent?: PluginChartNode) {
    ...
    const normalizedNode = normalizeTreeNode(
      node,
      parent,
      (a, b) => (b?.meta?.duration ?? 0) - (a?.meta?.duration ?? 0),
    );
  ...
  function createMetricsNodes(prefix: string, metrics: PluginBuildInfo[]) {
    const otherTypes = metrics.filter(
      (item) =>
        !moduleTypes.value.some((type) => getFileTypeFromModuleId(item.module!).name === type.name),
    );
    return [
      ...moduleTypes.value.map((type, idx) => {
        const filteredMetrics = metrics.filter(
          (item) => getFileTypeFromModuleId(item.module!).name === type.name,
        );
        return createNode({
          id: `${prefix}-${type.name}-${idx}`,
          text: type.description,
          children: filteredMetrics.map((item, idx) =>
            createNode({
              id: `${prefix}-${idx}`,
              text: item.module,
              size: item.duration,
      ...
      createNode({
        id: `${prefix}-other`,
        text: 'Other',
        children: otherTypes.map((item, idx) =>
          createNode({
            id: `${prefix}-other-${idx}`,
            text: item.module,
            size: item.duration,
...
const { tree, chartOptions, graph, nodeHover, nodeSelected, selectedNode, selectNode, buildGraph } =
  useChartGraph<PluginChartInfo, PluginChartInfo, PluginChartNode>({
    data: [],
    tree: _tree,
    nameKey: 'id',
    sizeKey: 'duration',
...
watch(
  () => settings.value.pluginDetailsViewType,
  () => {
    buildGraph();
  },
);
...
<template>
  <VisualLoading v-if="!state" />
  <div v-else-if="state?.calls?.length" relative h-full w-full>
    <DisplayCloseButton absolute right-2 top-1.5 @click="emit('close')" />
    <div
      bg-glass
      absolute
      left-2
    ...
    >
      ...
      <div text-xs font-mono flex="~ items-center gap-3" ml2>
        <DisplayDuration
          :duration="hookResolveIdDuration"
        ...
        <DisplayDuration
          :duration="hookTransformDuration"
          flex="~ gap-1 items-center"
        ...
        <span op40>|</span>
        <DisplayNumberBadge
          :number="processedModules.length"
