...
import DisplayTimestamp from '../../../_devtools-ui/components/DisplayTimestamp.vue';
...
const props = withDefaults(
  ...
  {
    fields: () => ['event_id', 'action', 'module_id', 'plugin_name', 'timestamp', 'content', '*'],
  },
);
...
function omit(obj: RolldownEvent, inputs: string[]) {
  ...
  return Object.fromEntries(Object.entries(obj).filter(([key]) => !fields.has(key)));
}

function getContent(event: RolldownEvent) {
  if (
    event.action === 'HookLoadCallEnd' ||
    event.action === 'HookTransformCallStart' ||
...
<template>
  <div v-if="!events.length">
    <div p5 italic>No events found</div>
  </div>
  <table>
    <tbody>
      <tr v-for="event of props.events" :key="event.event_id">
        <template v-for="field of props.fields" :key="field">
          <td px2>
            ...
            <DisplayTimestamp
              v-else-if="field === 'timestamp' && 'timestamp' in event"
              text-sm
