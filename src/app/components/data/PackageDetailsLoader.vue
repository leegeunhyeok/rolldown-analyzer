...
const { getPackageDetails } = useData();
const state = computed(() => getPackageDetails(props.package));

const normalizedBundledFiles = computed(
  () => state.value?.files?.filter((f) => !!f.transformedCodeSize) ?? [],
);

const importers = computed(() => {
  const pathMap = new Map();
  state.value?.files
    .filter((f) => !!f.importers)
    .flatMap((f) => f.importers)
    .filter((i) => !i.path.startsWith(state.value?.dir ?? ''))
    .forEach((importer) => {
