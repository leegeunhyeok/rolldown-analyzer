...
createModuleGraph<ModuleListItem, ModuleImport>({
  ...
  generateGraph: (options) => {
    ...
    return (focusOnFirstRootNode = true) => {
        ...
        (parent) => {
          ...
          const modules = parent.module.imports
            .map((x): ModuleGraphNode<ModuleListItem, ModuleImport> | undefined => {
              ...
              if (
                childToParentMap.has(module.id) &&
                childToParentMap.get(module.id) !== parent.module.id
              )
                return undefined;
      ...
      const layout = tree<ModuleGraphNode<ModuleListItem, ModuleImport>>().nodeSize([
      ...
      for (const node of _nodes) {
        ...
        if (node.data.module.imports) {
          node.data.hasChildren =
            node.data.module.imports?.filter(
              (subNode) => childToParentMap.get(subNode.module_id) === node.data.module.id,
            ).length > 0;
        }
      }
