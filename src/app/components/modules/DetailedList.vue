...
const filteredTransformsMap = computed(() => {
  ...
  for (const mod of props.modules) {
    const t = mod.buildMetrics?.transforms
      .filter((i) => i.source_code_size !== i.transformed_code_size)
      .filter((i) => i.transformed_code_size);
    map.set(mod.id, t!);
  }
  return map;
});
...
<template>
  <div flex="~ col gap-2" p4>
    <DataVirtualList :items="modules" key-prop="id">
              ...
              <div flex="~ gap-1 wrap" text-xs>
                <ul flex="~ auto text-xs wrap">
                  <template v-for="(p, i) of filteredTransformsMap.get(item.id)" :key="i">
                    <li
                      v-if="
                        p.source_code_size !== p.transformed_code_size && p.transformed_code_size
                      "
                      flex="~ items-center"
                    >
                      <DisplayPluginName :name="p.plugin_name" class="font-mono ws-nowrap op-50" />
                      <span v-if="i !== filteredTransformsMap.get(item.id)!.length - 1" op20
                        >|</span
                      >
                    </li>
                  </template>
