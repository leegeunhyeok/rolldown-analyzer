...
createModuleGraph<ChunkInfo, ChunkImport>({
  ...
  generateGraph: (options) => {
    ...
    return () => {
      ...
      const entryChunks = chunks.value.filter((chunk) =>
        props.entryId ? chunk.id === props.entryId : chunk.reason === 'entry',
      );
        ...
        (parent) => {
          ...
          const nodes = parent.module.imports
            ?.map((x): ModuleGraphNode<ChunkInfo, ChunkImport> | undefined => {
              ...
              if (!module || seen.has(module)) return undefined;
      ...
      const layout = tree<ModuleGraphNode<ChunkInfo, ChunkImport> & { end?: boolean }>().nodeSize([
        unref(spacing.height),
        unref(spacing.width) + unref(spacing.gap),
      ]);
      layout(root);
      ...
      const _additionalLinks = chunks.value.flatMap((chunk) =>
        chunk.imports
          .filter(
            (_import) => !_links.find((x) => x.id === `${chunk.chunk_id}|${_import.chunk_id}`),
          )
          .map((_import) => ({
...
<template>
  <DisplayModuleGraph :session="session" :modules="chunks" :expand-controls="false">
    <template #default="{ node }">
      <NuxtLink
        class="flex items-center"
        :to="{ path: route.path, query: { chunk: node.data.module.chunk_id } }"
      >
