...
<template>
  <div role="table" min-w-max border="~ base rounded-xl">
    <div role="row" class="border-b border-base" flex="~ row">
      <div
        :title="groupView ? 'Package' : 'Bundled packages'"
        role="columnheader"
        rounded-tl-2
        flex-none
        ws-nowrap
        ...
        px2
        font-600
        :class="[groupView ? 'min-w40' : 'min-w80']"
      >
      ...
      >
        <button flex="~ row gap1 items-center justify-end" w-full relative pr2>
          ...
          <span
            ...
            @click="toggleSizeSortType"
          >
            <i
              text-xs
              :class="[
                sizeSortType !== 'asc' ? 'i-ph-arrow-down-duotone' : 'i-ph-arrow-up-duotone',
                sizeSortType ? 'op100 text-primary' : 'op50',
              ]"
            />
          </span>
        </button>
    ...
    <DataVirtualList v-if="packages.length" role="rowgroup" :items="packages" key-prop="dir">
      <template #default="{ item, index }">
        ...
        >
          <Tooltip :triggers="['hover']" :delay="1200" :disabled="item.name.length < 30">
            <div
              v-if="!groupView"
              role="cell"
              font-mono
          ...
          <div
            ...
            min-w40
            op80
            :class="{ 'text-primary': item.duplicated }"
          >
            {{ item.version }}
          </div>
          <div
            ...
            <VMenu :delay="{ show: 200, hide: 0 }">
              ...
              <template #popper>
                <div p2 flex="~ col gap-1">
                  <div
                    v-for="file of item.files.filter((f) => !!f.transformedCodeSize)"
                    :key="file.path"
                    flex="~ row gap-1 items-center nowrap"
                    w-max
                  >
