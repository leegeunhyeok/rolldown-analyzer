...
const importers = computed(() => {
  ...
  props.package.files
    ...
    .filter((i) => !i.path.startsWith(props.package?.dir ?? ''))
    .forEach((importer) => {
...
<template>
  <div flex="~ row gap-1" of-hidden>
    <div flex="~ row gap-1" ws-nowrap>
      ...
      <DisplayBadge
        v-if="importers[0]!.version && showVersion"
        :text="importers[0]!.version"
        as="span"
      />
    </div>
      ...
      <template #popper>
        <div p2 flex="~ col gap-1">
          ...
          >
            ...
            />
            <DisplayBadge
              v-if="importer.version && showVersion"
              :text="`v${importer.version}`"
              as="span"
            />
          </div>
        </div>
