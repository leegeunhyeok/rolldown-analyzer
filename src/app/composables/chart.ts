...
export function useChartGraph<
  T extends Record<string, any>,
  I extends T & Record<string, any>,
  N extends TreeNode<any>,
>(options: ChartGraphOptions<T, I, N>) {
  ...
  const graph = shallowRef<GraphBase<I | undefined, GraphBaseOptions<I | undefined>> | undefined>(
    undefined,
  );
  ...
  function selectNode(node: N | null, animate?: boolean) {
    ...
    if (!node?.children.length) node = node?.parent as unknown as N | null;
  ...
  function buildGraph() {
    ...
    nextTick(() => {
      const selected = selectedNode.value
        ? tree.value.map.get(selectedNode.value[nameKey]) || null
        : null;
