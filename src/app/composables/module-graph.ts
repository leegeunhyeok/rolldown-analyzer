...
interface ModuleGraphOptions<M, I> {
  ...
  generateGraph: (
    options: ModuleGraphGenerateBaseOptions & {
      nodes: ShallowRef<
        HierarchyNode<ModuleGraphNode<M, I>>[],
        HierarchyNode<ModuleGraphNode<M, I>>[]
      >;
...
const ViteDevToolsModuleGraphStateSymbol: InjectionKey<
  ModuleGraphGenerateBaseOptions & {
    nodes: ShallowRef<
      HierarchyNode<ModuleGraphNode<any, any>>[],
      HierarchyNode<ModuleGraphNode<any, any>>[]
    >;
...
export function generateModuleGraphLink<M, I>(
  link: ModuleGraphLink<M, I>,
  spacing?: ModuleGraphSpacing,
) {
  ...
  if (!spacing) {
    ...
    if (link.target.x! <= link.source.x!) {
      return createLinkVertical({
        source: [
          link.source.x! + unref(spacing.width) / 2 - unref(spacing.linkOffset),
          link.source.y!,
        ],
        target: [
          link.target.x! - unref(spacing.width) / 2 + unref(spacing.linkOffset),
          link.target.y!,
        ],
      });
    }
...
export function useToggleGraphNodeExpanded<M extends { id: string; imports: unknown[] }>(options: {
  ...
  const { nodesRefMap, container, calculateGraph, collapsedNodes } = inject(
    ViteDevToolsModuleGraphStateSymbol,
  )!;
  ...
  function toggleNode(id: string) {
    if (isGraphNodeToggling.value) return;
