...
const { pathSelectorVisible, pathNodes, selectPathNodes, togglePathSelector, normalizedGraph } =
  useGraphPathManager<RolldownChunkInfo & { id: string }>({
    onToggle: (visible) => {
...
function getChunkSize(chunk: RolldownChunkInfo): number {
  ...
  return chunk.modules.reduce((total, id) => {
    ...
    if (!moduleInfo || !moduleInfo.buildMetrics?.transforms?.length) return total;
...
const { tree, chartOptions, graph, nodeHover, nodeSelected, selectedNode, selectNode, buildGraph } =
  useChartGraph<Omit<RolldownChunkInfo, 'type'>, ChunkChartInfo, ChunkChartNode>({
    data: chunksWithSize,
    nameKey: 'filename',
    sizeKey: 'size',
    rootText: 'Chunks',
  ...
  <div relative max-h-screen of-hidden>
    ...
    <template v-if="settings.chunkViewType === 'list'">
      <div class="px5 pt32 of-auto h-screen" flex="~ col gap-4">
        <ChunksFlatList :session="session" :chunks="searched" />
      </div>
    </template>
    ...
    <template v-else-if="settings.chunkViewType === 'flamegraph'">
      <div of-auto h-screen flex="~ col gap-2" pt32>
        <ChunksFlamegraph v-if="graph" :graph="graph" />
      </div>
    </template>
