...
const { tree, chartOptions, graph, nodeHover, nodeSelected, selectedNode, selectNode, buildGraph } =
  useChartGraph<Omit<RolldownAssetInfo, 'type'>, AssetChartInfo, AssetChartNode>({
    data: searched,
    nameKey: 'filename',
    sizeKey: 'size',
    rootText: 'Project',
    ...
    graphOptions: {
      ...
      onHover(node) {
        if (node && !route.query.asset) nodeHover.value = node;
  ...
  <div relative max-h-screen of-hidden>
    ...
    <div of-auto h-screen flex="~ col gap-2" pt32>
      ...
      <template v-else-if="settings.assetViewType === 'flamegraph'">
        <AssetsFlamegraph v-if="graph" :graph="graph" />
      </template>
    </div>
