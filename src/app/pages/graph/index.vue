...
const searchValue = ref<{
  ...
  node_modules: (route.query.node_modules ? toArray(route.query.node_modules) : null) as
    | string[]
    | null,
});
...
const filtered = computed(() => {
  ...
  if (searchValue.value.node_modules) {
    modules = modules.filter(
      (mod) => mod.path.moduleName && searchValue.value.node_modules!.includes(mod.path.moduleName),
    );
  }
  ...
  <div relative max-h-screen of-hidden>
      ...
      <DataSearchPanel v-model="searchValue" :rules="searchFilterTypes">
        ...
        <div flex="~ gap-2 items-center" p2 border="t base">
          ...
          <button
            ...
            btn-action
            :class="
              settings.moduleGraphViewType === viewType.value ? 'bg-active' : 'grayscale op50'
            "
    ...
    <template v-if="settings.moduleGraphViewType === 'list'">
      <div of-auto h-screen pt-45>
        <ModulesFlatList :session="session" :modules="searched" />
        <div
          absolute
          bottom-4
          py-1
    ...
    <template v-else-if="settings.moduleGraphViewType === 'detailed-list'">
      <div of-auto h-screen pt-45>
        <ModulesDetailedList :session="session" :modules="searched" />
        <div
          absolute
          bottom-4
          py-1
    ...
    <template v-else-if="settings.moduleGraphViewType === 'graph'">
      <ModulesGraph :session="session" :modules="normalizedGraph" />
    </template>
    <template v-else>
      <ModulesFolder :session="session" :modules="searched" />
    </template>
  </div>
</template>
